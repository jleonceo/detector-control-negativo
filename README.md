...
Una suite que solo comprueba que lo bueno pasa aprueba igual al código bueno y al roto. Esta
...
Aquí, **suite** es cada uno de esos ficheros, y **brazo negativo** es el caso que uno de
ellos mete a propósito para que falle. Sin brazo negativo, una suite da verde con el
código bueno y con el roto, así que su verde no informa de nada. El programa, en sus salidas y en
la clave del YAML, los llama `banco`.
...
[![ci](https://github.com/jleonceo/detector-control-negativo/actions/workflows/ci.yml/badge.svg)](https://github.com/jleonceo/detector-control-negativo/actions/workflows/ci.yml)
...
**Los ocho suites de `ejemplo/bancos/` están plantados a propósito, uno a uno, con su etiqueta al
lado.** Dos sin brazo negativo, que son los dos que el informe señala; cinco con brazo negativo,
uno por cada señal; y un agregador, para el tercer valor de la etiqueta. El noveno suite del
...
pruebas del propio detector y sale verde sin tocar una sola fixture.
...
El caso más común es una suite que comprueba solo el camino bueno: se le pasa una entrada
correcta, se comprueba que sale el resultado correcto y ahí acaba. Esa suite da verde. Y sigue
dando verde cuando alguien rompe la validación de entrada, porque nadie le pregunta nunca qué
hace con una entrada mala. En la jerga es una suite sin **control negativo**: le falta el caso que
afirma lo que la herramienta debe rechazar, bloquear, dejar vacío o marcar como inválido.

Contar cuántas suites están en ese estado parece un ejercicio de una tarde. Lo intentamos y salió
así:

| versión del criterio | qué añadió | suites sin control negativo |
|---|---|---:|
...
python skills/detector-control-negativo/verificar_control_negativo.py \
  ...
  --etiquetas Aplicaciones/_gobernanza/detector_control_negativo/etiquetas_control_negativo.yaml
```

Los tres suites son `Aplicaciones/05_capa_confianza_rag/suite_aceptacion/test_c11_determinismo.py`,
...
3. **Los dos «131» de la tabla no son el mismo objeto.** El de la fila v1 es el universo sin
   ...
   son 131 suites con casos propios **más** 5 agregadores **más** 3 excluidos. Y el denominador se
   movió ese mismo día: entraron tres suites al árbol a las 12:34, a las 13:02 y a las 14:18.
...
Un fichero de etiquetas escrito a mano, abriendo cada suite, con tres campos por entrada:
...
se llaman como una suite y no tienen un solo caso propio, porque descubren otros y los lanzan.
...
| `3` | no hay universo: ninguna raíz de git, o cero suites encontradas |

**El recuento de suites se reporta y no tumba nada.** Es la consecuencia directa de la tabla de
...
suites sin control negativo» ahí sería el falso verde de manual: el mensaje más tranquilizador
...
Basta una para que la suite cuente como cubierto.
...
sin esta señal sus suites salían como carentes. Fue uno de los dos falsos positivos que destapó
la revisión a mano.

**Las señales de texto se buscan donde toca y no en cualquier cadena.** La primera versión
miraba «no» y «sin» dentro de todo el fichero y **disparaba en 129 suites de 129**. Una señal que
...
una verificada mordida a mordida **antes** de mirar qué decía la suite. Es lo contrario de
...
- Dos mordieron limpio. Al romper el reconocimiento de agregadores (el fallo que en la primera
  ...
  señalando con nombre y apellido la suite mal clasificado.
- Dos no rompieron nada. Ahí estaba el agujero. Al borrar el literal `_CN_` del
  ...
  **no cayó ni un test**. Cada una de esas dos formas dependía en exclusiva de un único suite del
  árbol medido. Ninguno de esos dos suites estaba en el fichero de etiquetas. Una suite de 24
...
La suite pasó de 24 a 26 casos.
...
que en el árbol privado no existía o estaba clavada dentro del script: que las suites plantadas a
propósito se cacen, que ninguna suite de este repositorio se quede sin etiqueta, que la raíz de git
...
El día de publicar había tres verdes. La suite en 32. La mutación en 6 de 6. El acuerdo con las
etiquetas en 9 de 9. Se le pidió entonces a un auditor externo que buscara con material que **no
estaba en la suite**. Encontró dos defectos y ninguno de los tres verdes podía verlos.
...
Eso obliga a decir algo incómodo de la propia herramienta: **este detector cuenta qué suites
tienen un brazo negativo, y tener un brazo negativo no es discriminar.** Una suite puede llevar
`assertFalse` y seguir pasando con el código roto. Lo que separa una suite de un adorno es mutar
...
suites con nombre y apellido, en la fila que la deja donde le toca: dentro de una serie que baja en
cada afinado.

Lo que sí puedes ejecutar es el ejemplo del principio, sobre este mismo repositorio y sin preparar
nada. Da **9 etiquetas de 9 suites, concordancia 9 de 9 y salida `0`**. El «SIN control negativo
2 de 8» que sale ahí describe el árbol de juguete de `ejemplo/bancos/`, plantado suite a suite.
...
se mueve, mientras el recuento se mide contra un árbol al que ese día le entraron tres suites. Es
...
1. **Las etiquetas negativas son pocas.** Tres casos de «no lo tiene» no permiten afirmar una tasa
   ...
   suite sin que una etiqueta lo respalde.
2. **El universo es lo que git publica y el criterio es el nombre.** Una suite del árbol de
   trabajo que nadie ha añadido queda fuera. Una suite que no se llame `test_*.py`,
...
Ese rato es el trabajo. No hay atajo: medir una suite obliga a leerlo.
...
| `--listar` | cada suite con su veredicto y cada veredicto con la señal que lo disparó |
| `--etiquetar` | plantilla YAML de las suites que aún no tienen etiqueta |
...
**38 casos. Siete de ellos son controles negativos marcados `CN`.** En una suite cuya objeto es
cazar suites sin control negativo, no tenerlos sería la ironía más cara del repositorio. El que
...
marcando 129 suites de 129.

La suite mide dos objetos distintos a propósito. Los casos de fixture prueban el criterio con
...
direcciones**: ninguna etiqueta puede apuntar a un fichero que ya no existe, y ninguna suite puede
quedarse sin etiquetar. Sin la segunda, alguien añade una suite, no lo etiqueta, y la puntuación
...
suite con casos que juzgar. Contaminar la medición con el instrumento es la clase de error que
este repositorio persigue.

### Requisitos

Python 3.9 o superior, biblioteca estándar, sin red y sin nada que instalar. Ese «nada que
instalar» cubre el instrumento y su batería de 38 casos, que se ejecutan con `python` a secas. El
único comando de esta página que pide algo de fuera es el `pytest` con el que se comprueba un clon
recién hecho, y el propio CI lo instala aparte. En macOS y en casi
...
Ese 3.9 estuvo **declarado y sin certificar** hasta el 31/07/2026. Esta nota se corrige en vez de
borrarse porque así estaba prometido. En local solo se había ejecutado con Python 3.10, 3.13 y 3.14,
las tres sobre Windows, o sea en ninguna versión por debajo de 3.10, que era la que importaba. Un
repaso del código no encontraba sintaxis ni biblioteca por encima de 3.9, pero eso encuentra
incompatibilidades sin certificar compatibilidad.

Lo que la certifica es la matriz de CI, y el primer push la puso en verde entera: **nueve
combinaciones, 3.9, 3.11 y 3.13 sobre Windows, Linux y macOS**, más el trabajo que compara este
repositorio con su gemelo. El 3.9 ya está certificado por ejecución.
...
- **[adherencia-reglas](https://github.com/jleonceo/adherencia-reglas)**: cuenta qué fracción de
  ...
  mide una norma, aquí se mide una suite.
...
>
> **Two words used throughout.** A *test suite* is what the tool calls `banco`, the Spanish for
> bench. Its *negative arm* is the case a test suite includes on purpose so that it fails; without
> one, a test suite stays green over working code and over broken code alike, so its green says
> nothing.
...
The most common case is a test suite that only checks the happy path: feed it a valid input, assert the
correct result, done. That test suite is green. It stays green after somebody breaks input validation,
...
Counting how many test suites are in that state looks like an afternoon's work. Here is how it went:

| criterion version | what it added | test suites with no negative control |
|---|---|---:|
...
3. **The two "131" in the table are not the same object.** The one in row v1 is the undifferentiated
   ...
   row is 131 test suites with cases of their own **plus** 5 aggregators **plus** 3 excluded. And the
   denominator moved that very day: three test suites entered the tree at 12:34, at 13:02 and at 14:18.

**None of the versions was a coding bug.** Each refinement measured a different object, and each returned less
...
A label file written by hand, opening every test suite, three fields per entry:

```yaml
  - banco: ejemplo/bancos/test_veredicto_malo.py     # test file
...
are named like a test suite and hold no case of their own, because they discover other test suites and
...
| `3` | no universe: no git root, or zero test suites discovered |

**The test suite count is reported and gates nothing.** That follows straight from the table above:
...
test suites without a negative control" there would be the textbook false green: the most reassuring
...
One is enough for the test suite to count as covered.
...
without this signal its test suites came out as uncovered. That was one of the two false positives
the manual review exposed.

**Text signals are looked for where they belong and not inside any string.** The first version
looked for "no" and "sin" across the whole file and **fired on 129 test suites out of 129**. A signal
...
one verified to bite **before** looking at what the test suite said. This is the opposite of boasting
...
- Two broke nothing. That was the hole. Deleting the `_CN_` literal from marker
  ...
  test**. Each of those two forms depended exclusively on a single test suite in the measured tree.
  Neither of those test suites was in the label file. A 24-case test suite, green, and two lines of the
...
The test suite went from 24 to 26 cases.
...
did not exist or was hardcoded in the script: the deliberately planted test suites getting caught, no
test suite of this repository going unlabelled, the git root never invented where there is none, the
...
On publication day there were three greens. The test suite at 32. Mutation at 6 of 6. Label agreement at
9 of 9. An outside auditor was then asked to look with material that was **not in the test suite**. It
...
test suites have a negative arm, and having a negative arm is not the same as discriminating.** A
test suite can carry `assertFalse` and still pass with broken code. What separates a test suite from an
...
its date, its hour, its commit and the three test suites named one by one, in the row that puts it where
...
It gives **9 labels over 9 test suites, agreement 9 of 9 and exit `0`**, and the "SIN control negativo 2
de 8" printed there describes the toy tree in `ejemplo/bancos/`, planted test suite by test suite. Two with no
negative arm, which are the two the report flags; five with one, one per signal; and one aggregator,
for the label's third value. The ninth test suite of that universe is the tool's own.
...
collects 38 tests, all 38 from the detector's own test suite, and comes out green without touching a
single fixture.
...
test suites that same day. That is the difference between the two figures. It is also the whole thesis
of the tool.
...
1. **There are few negative labels.** Three "it does not have one" cases do not support a false
   negative rate. What is measured is that the new part of the criterion rescues no test suite without
   a label backing it.
2. **The universe is what git publishes, and the criterion is the filename.** A test suite in the
   working tree that nobody added stays out. A test suite not named `test_*.py`, `run_tests*.py` or
...
That time is the work. There is no shortcut: measuring a test suite means reading it.
...
| `--listar` | every test suite with its verdict, and every verdict with the signal that fired |
| `--etiquetar` | a YAML template for the test suites that have no label yet |

The root comes from git (`git rev-parse --show-toplevel`) rather than deduced by counting
...
**38 cases, seven of them negative controls marked `CN`.** In a test suite whose whole purpose is
catching test suites without negative controls, having none would be the most expensive irony in the
...
version ended up marking 129 test suites out of 129.

The test suite measures two different objects on purpose. The fixture cases test the criterion against
...
point at a file that no longer exists, and no test suite may go unlabelled. Without the second one,
somebody adds a test suite, does not label it, and the score keeps coming out perfect over the ones
...
so calling it `test_*` dropped it into its own universe as though it were a test suite with cases to
judge. Contaminating the measurement with the instrument is the kind of mistake this repository is
about.

### Requirements

Python 3.9 or newer, standard library, no network and nothing to install. That "nothing to
install" covers the instrument and its 38-case bank, which run under plain `python`. The only
command on this page that needs anything external is the `pytest` that checks a fresh clone, and
the CI installs it separately. On macOS and most Linux
the interpreter is `python3`, not `python`: the commands on this page say `python` because they
were written on Windows.

That sentence describes the **instrument**. It is worth making stricter, because the repository does not
...
That 3.9 was **declared and uncertified** until 31/07/2026. This note gets corrected instead of
deleted because that is what it promised. Locally it had only been run with Python 3.10, 3.13 and
3.14, all three on Windows, and on nothing below 3.10, which was the version that mattered. A
read-through found no syntax or library above 3.9, but that finds incompatibilities without
certifying compatibility.

What certifies it is the CI matrix, and the first push turned all of it green: **nine combinations,
3.9, 3.11 and 3.13 across Windows, Linux and macOS**, plus the job that compares this repository
with its twin. The 3.9 floor is now certified by execution.
...
- **[adherencia-reglas](https://github.com/jleonceo/adherencia-reglas)**: counts what fraction of
  ...
  a rule is measured, here a test suite.
